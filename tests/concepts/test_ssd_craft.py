...
def test_model_outputs(model_data):
    ...
    # SSD returns a list of dicts with detection results
    assert isinstance(processed_results, list), "SSD should return a list"
...
def test_gradients_model_original(image_data, model_data):
    ...
    assert check, (
        "Model gradients should be computed successfully when calling the default SSD model."
    )
...
def latent_extractor_data(dataset_classes, model_data, device_param):
    ...
    latent_extractor = SSDExtractorBuilder.build(
        model,
...
def test_craft_reencode(image_data, dataset_classes, craft_data):
    ...
    # SSD spatial dimensions depend on extraction layer
    ...
    filtered_result = result.filter(confidence=0.3)
...
def test_craft_decoder_modes(image_data, dataset_classes, craft_data, device_param):
    ...
    # SSD output shape varies, just check it's not empty
