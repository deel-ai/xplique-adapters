...
def test_model_outputs(model_data):
    ...
    # FCOS returns a list of dicts with detection results
    assert isinstance(processed_results, list), "FCOS should return a list"
...
def test_gradients_model_original(image_data, model_data):
    ...
    assert check, (
        "Model gradients should be computed successfully when calling the default FCOS model."
    )
...
def latent_extractor_data(dataset_classes, model_data, device_param):
    ...
    latent_extractor = FcosExtractorBuilder.build(
        model,
...
def test_craft_reencode(image_data, dataset_classes, craft_data):
    ...
    # FCOS spatial dimensions depend on extraction location and layer
...
def test_craft_decoder_modes(image_data, dataset_classes, craft_data, device_param):
    ...
    # FCOS output shape varies, just check it's not empty
