"""
FCOS latent data extraction for concept-based explanations.

This module provides components for extracting and manipulating latent representations
from FCOS object detection models for use with Xplique's concept-based explanation methods.

See https://docs.pytorch.org/vision/main/_modules/torchvision/models/detection/fcos.html
...
class LatentDataFcos(LatentData):
    ...
    def set_activations(self, values: torch.Tensor) -> None:
        ...
        # Expand any other feature map levels that are still at batch_size=1
...
class FcosExtractorBuilder(LatentExtractorBuilder):
    ...
    def build(
        ...
        def g(self, images):
            ...
            # Determine which features to use based on extraction_location
        ...
        def h(self, latent_data: LatentDataFcos):
            ...
            # If extraction_location='resnet', we need to apply FPN here
            # If extraction_location='fpn', features are already FPN features
            if extraction_location == "fpn":
        ...
        print(
            f"Building FCOS extractor: extracting from '{extraction_location}' at layer index {extraction_layer}"
        )
