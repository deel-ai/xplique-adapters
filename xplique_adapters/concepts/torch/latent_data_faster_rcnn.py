"""
Faster R-CNN latent data extraction for concept-based explanations.

This module provides components for extracting and manipulating latent representations
from Faster R-CNN object detection models for use with Xplique's concept-based
explanation methods.
...
class LatentDataFasterRcnn(LatentData):
    ...
    def set_activations(self, values: torch.Tensor) -> None:
        ...
        # Expand any other feature map levels still at batch_size=1
        for key in self.features:
            if self.features[key].shape[0] != new_batch_size:
                self.features[key] = self.features[key][:1].expand(
    ...
    def to(self, device: torch.device) -> "LatentData":
        ...
        return LatentDataFasterRcnn(
            images, self.original_image_sizes, features, self.extraction_layer
        )
...
class FasterRcnnExtractorBuilder(LatentExtractorBuilder):
    ...
    def build(
        ...
        device: str = "cuda",
        nb_classes: int = 91,
        extraction_location: str = "resnet",
        extraction_layer: int = -1,
        batch_size: int = 1,
        ...
        def g(self, images):
            ...
            # Determine which features to pass based on extraction_location
            if extraction_location == "fpn":
        ...
        def h(self, latent_data: LatentDataFasterRcnn):
            ...
            # If extraction_location is 'resnet', features are ResNet features and need FPN
            # If extraction_location is 'fpn', features are already FPN features
            if extraction_location == "fpn":
            ...
            proposals, _ = self.rpn(images, fpn_features, targets=None)

            # Get detections from RoI heads
            detections, _ = self.roi_heads(
                fpn_features, proposals, images.image_sizes, targets=None
            )
        ...
        print(
            f"Building Faster R-CNN extractor: extracting from '{extraction_location}' at layer index {extraction_layer}"
        )
