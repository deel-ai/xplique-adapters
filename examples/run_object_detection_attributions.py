...
from xplique.attributions.gradient_statistics.vargrad import VarGrad
from xplique.attributions.gradient_statistics.smoothgrad import SmoothGrad
from xplique.attributions.gradient_statistics.square_grad import SquareGrad
...
MODEL_CONFIGS = {
    "detr": {
        ...
        "wrapper_args": {"image_size": (800, 800)},
    ...
    "yolo11": {
        ...
        "raw_wrapper_class": "Yolo11RawBoxesModelWrapper",
    ...
    "yolo26": {
        ...
        "image_size": (640, 640),
        "has_raw_wrapper": True,
        "raw_wrapper_class": "Yolo26RawBoxesModelWrapper",
...
def parse_args():
    ...
    model_choices = ["detr", "fasterrcnn", "retinanet", "fcos", "ssd", "yolo11", "yolo26"]
    parser.add_argument("--model", choices=model_choices, help="Single model to use")
    parser.add_argument(
        "--models", nargs="+", choices=model_choices,
        help="Models to run in one invocation (defaults to all models)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--methods", nargs="+", default=["occlusion", "saliency"],
                        choices=["occlusion", "saliency", "vargrad", "smoothgrad", "squaregrad"],
    ...
    args.models = args.models or ([args.model] if args.model else model_choices)
    return args
...
def create_wrapper(model_name, model, config, device, use_raw_wrapper=False):
    ...
    if use_raw_wrapper:
        ...
        if model_name == "detr":
            ...
            return DetrExtractorBuilder.build(model, device=str(device), image_size=config["image_size"])
        ...
        # Handle migrated wrapper classes
        if wrapper_class_name == "DetrBoxesModelWrapper":
            from xplique_adapters.object_detection.torch.detr_wrappers import DetrBoxesModelWrapper
            wrapper_class = DetrBoxesModelWrapper
        elif wrapper_class_name == "TorchvisionBoxesModelWrapper":
            from xplique_adapters.object_detection.torch.torchvision_wrappers import TorchvisionBoxesModelWrapper
            wrapper_class = TorchvisionBoxesModelWrapper
        elif wrapper_class_name == "YoloResultBoxesModelWrapper":
            from xplique_adapters.object_detection.torch.yolo_wrappers import YoloResultBoxesModelWrapper
            wrapper_class = YoloResultBoxesModelWrapper
        elif wrapper_class_name == "Yolo11RawBoxesModelWrapper":
            from xplique_adapters.object_detection.torch.yolo_wrappers import Yolo11RawBoxesModelWrapper
            wrapper_class = Yolo11RawBoxesModelWrapper
        else:
...
def run_model(args):
    ...
    CLASSES = COCO_CLASSES_80 if args.model in ["yolo11", "yolo26"] else COCO_CLASSES_91
    ...
    od_model = create_wrapper(args.model, model, config, device, use_raw_wrapper=False)
    results = od_model(processed_input)

    # Setup box manager
    box_type = BoxType(
        config["box_format"], is_normalized=config["normalized"]
    )
    ...
    for class_name in args.classes:
        ...
        # plot_image_detections() handles conversion internally via box_manager.to_numpy_tuple()
        class_boxes[class_name] = boxes

        print(f"\n{class_name}: {boxes.shape[0]} boxes")

        # Visualize boxes for this class
        # Note: plot_image_detections() accepts GPU tensors and converts internally
        if boxes.shape[0] > 0:
            fig = plot_image_detections(
                display_image,
                boxes,
    ...
    if hasattr(od_model, 'eval') and args.model not in ["yolo11", "yolo26"]:
        od_model.eval()
    ...
    for method in args.methods:
        ...
        try:
            ...
            if method == "occlusion":
            ...
            elif method in ["saliency", "vargrad", "smoothgrad", "squaregrad"]:
                if not gradient_check:
                    print(f"WARNING: Skipping {method.capitalize()} due to gradient check failure")
                    if args.model == "detr":
                        print(
                        ...
                        print(
                            "NOTE: This is expected for YOLO11/YOLO26 - "
                ...
                if method == "vargrad":
                    ...
                    explainer_class = SmoothGrad
                elif method == "squaregrad":
                    explainer_class = SquareGrad
                else:
                    explainer_class = Saliency
    ...
    needs_phase2 = (args.model == "detr" or args.model in ["yolo11", "yolo26"])
    ...
    if run_phase2:
        ...
        alt_model.output_as_tensor = True
        ...
        elif args.model in ["yolo11", "yolo26"]:
            # YOLO: Set training flag to False for TorchWrapper compatibility
            alt_model.training = False
            alt_wrapped_model = TorchWrapper(alt_model, device=device, is_channel_first=True, requires_grad=True)
        ...
        for method in args.methods:
            ...
            if method == "occlusion":
                ...
                torch_wrapped_model = alt_wrapped_model
                if method == "vargrad":
                    explainer_class = VarGrad
                elif method == "smoothgrad":
                    explainer_class = SmoothGrad
...
def main():
    args = parse_args()
    run_name = datetime.now().strftime("object_detection_attributions_%Y%m%d_%H%M%S")
    output_root = args.output_dir or run_name

    for model_name in args.models:
        args.model = model_name
        args.output_dir = os.path.join(output_root, model_name)
        run_model(args)
