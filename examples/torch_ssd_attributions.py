"""
Attribution Methods on SSD with PyTorch

This script demonstrates multiple attribution methods on an SSD (Single Shot MultiBox Detector) object detection model.
...
# Output directory
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "torch_ssd_attributions")
...
# SSD300 expects 300x300 input, but we'll resize for memory efficiency
...
# Get COCO class names from SSD weights
CLASSES = weights.meta["categories"]
nb_classes = len(CLASSES)
print(f"Loaded {nb_classes} COCO class names")
class_id = CLASSES.index(class_name)

# Preprocess images using SSD transforms
...
for img in raw_images:
    ...
    img_tensor = preprocess(img_tensor).to(device)
    processed_inputs.append(img_tensor)
...
od_model = TorchvisionBoxesModelWrapper(model, nb_classes)
...
# Set model to output tensors for attribution
od_model.output_as_tensor = True

# Wrap model for attributions using xplique's TorchWrapper
explainers = {}
...
for img_idx, (raw_image, processed_input, filtered_boxes) in enumerate(
    ...
    # Convert inputs to TensorFlow format (required by Xplique)
...
print("\n" + "=" * 80)
print("6. Creating Combined Visualization")
print("=" * 80)
