...
from xplique.utils_functions.object_detection.base.box_manager import BoxFormat, NumpyBoxManager
import tensorflow as tf

# Prevent TensorFlow from pre-allocating all GPU memory
_tf_gpus = tf.config.experimental.list_physical_devices('GPU')
...
def parse_args():
    ...
    parser.add_argument("--extraction_location", type=str, default="resnet",
                        choices=["resnet", "fpn"],
                        help=(
    ...
    parser.add_argument("--feature-viz-total-steps", type=int, default=100,
                        help="Optimization steps per feature visualization (default: 100)")
    parser.add_argument("--feature-viz-crops-per-iteration", type=int, default=2,
                        help="Crops optimized per iteration (default: 2)")
...
def build_latent_extractor(model_name, model, config, device, framework="torch",
    ...
    if framework == "tf":
        ...
        if model_name == "retinanet":
            ...
            return RetinaNetExtractorBuilder.build(
                ...
                index_activations=config["extraction_layer"],
    ...
    if model_name in ["yolo11", "yolo26"]:
        ...
        mode = (
            ...
            else YoloExtractorMode.ONE_TO_MANY
        )
...
def preprocess_images(raw_images, model_name, config, weights=None, framework="torch"):
    ...
    elif model_name in ["yolo11", "yolo26"]:
        if config["image_size"] is not None:
            transform = T.Compose([
...
def plot_concepts_feature_viz(craft, _input_tensor, indices_to_plot, device,
                              ...
                              crops_per_iteration=2, total_steps=100):
    ...
    for i, idx in enumerate(indices_to_plot):
        image1, alpha1 = maco(objective_for_concept(idx), model_input_size=model_input_size,
                             crops_per_iteration=crops_per_iteration, device=device,
...
def run_craft(args, output_dir=None, device_override=None):
    ...
    extraction_location_suffix = (
        "" if args.extraction_location == "resnet"
        else f"_{args.extraction_location}"
    )
    ...
    output_dir_name = (
        ...
        f"{nmf_suffix}{extraction_location_suffix}"
    ...
    if args.framework == "torch":
        device = device_override or torch.device(
    ...
    latent_extractor = build_latent_extractor(
        ...
        framework=args.framework,
        extraction_location=args.extraction_location
    )
    ...
    _fig = plot_image_detections(
        display_image, box_to_display,
    ...
    ### New computing: Prevalence and Reliability    
    ...
    # Plot importance bars for Gradient Input
    plt.figure(figsize=(10, 6))
    plt.bar(list(range(len(importances_gradient_input))), importances_gradient_input)
    plt.xlabel('Concept ID')
    plt.ylabel('Importance')
    plt.title(f'Concept Importances (Gradient Input) for "{args.class_name}" class')

    output_path = os.path.join(output_dir, "05_importance_bars_gi2.png")
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"Saved: {output_path}")
    plt.close()
    ...
    # Plot prevalence bars
    plt.figure(figsize=(10, 6))
    plt.bar(list(range(len(prevalence))), prevalence)
    plt.xlabel('Concept ID')
    plt.ylabel('Prevalence')
    plt.title(f'Concept Prevalence for "{args.class_name}" class')

    output_path = os.path.join(output_dir, "05_prevalence_bars.png")
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"Saved: {output_path}")
    plt.close()
    ...
    # reliability
    # Compute per-image accuracy as max IoU between predicted boxes (class_id, conf)
    # and COCO ground-truth boxes for the target class, using xplique's _box_iou.
    #
    # Predicted boxes are in XYXY format, absolute pixel coords in the resized image space.
    # COCO GT boxes are in XYWH format, original image pixel coords → scaled to resized space.
    print("\nComputing per-image IoU accuracy against COCO ground truth...")
    accuracy = []
    class_id = classes_names.index(args.class_name)
    images_size = config["image_size"]

    for i, img_id in enumerate(selected_img_ids):
        img_tensor = input_tensor[i : i + 1]
        with torch.no_grad():
            res = latent_extractor(img_tensor)

        pred_filtered = res[0].filter(class_id=class_id, confidence=config["confidence"])
        pred_boxes_np = pred_filtered.boxes().cpu().numpy()  # XYXY, resized pixel coords

        # Load COCO GT annotations for this image and class
        ann_ids = coco.getAnnIds(imgIds=img_id, catIds=cat_ids, iscrowd=False)
        anns = coco.loadAnns(ann_ids)

        if len(pred_boxes_np) == 0 or len(anns) == 0:
            accuracy.append(0.0)
            continue

        # Convert GT boxes from XYWH (original pixel coords) to XYXY (resized pixel coords)
        img_info = coco.loadImgs(img_id)[0]
        orig_w, orig_h = img_info["width"], img_info["height"]
        scale = np.array([images_size[0] / orig_w, images_size[1] / orig_h,
                          images_size[0] / orig_w, images_size[1] / orig_h])

        gt_xywh = np.array([ann["bbox"] for ann in anns], dtype=np.float32)
        gt_xyxy = NumpyBoxManager.box_xywh_to_xyxy(gt_xywh) * scale

        # Compute pairwise IoU using xplique's _box_iou
        # _box_iou supports broadcasting: pred (N, 1, 4) vs gt (1, M, 4) → (N, M)
        pred_tf = tf.constant(pred_boxes_np[:, None, :], dtype=tf.float32)
        gt_tf = tf.constant(gt_xyxy[None, :, :], dtype=tf.float32)
        iou_matrix = _box_iou(pred_tf, gt_tf)  # (N, M)

        max_iou = float(tf.reduce_max(iou_matrix).numpy())
        accuracy.append(max_iou)

    print(f"Accuracy (max IoU) computed for {len(accuracy)} images")
    print(f"Mean accuracy: {np.mean(accuracy):.4f}")

    reliability = craft.reduce_to_reliability(explanation, accuracy=accuracy)
    print(f"\nReliability scores: {reliability}")

    reliability_order = reliability.argsort()[::-1]
    print(f"Reliability concept ranking: {reliability_order}")

    # Plot reliability bars
    plt.figure(figsize=(10, 6))
    plt.bar(list(range(len(reliability))), reliability)
    plt.xlabel('Concept ID')
    plt.ylabel('Reliability')
    plt.title(f'Concept Reliability for "{args.class_name}" class')

    output_path = os.path.join(output_dir, "05_reliability_bars.png")
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"Saved: {output_path}")
    plt.close()
    ...
    if args.skip_feature_viz:
        ...
        import gc
        # Move input_tensor to CPU to free VRAM — it is not needed for feature viz
        input_tensor = input_tensor.cpu()
        torch.cuda.empty_cache()
        gc.collect()
        print(f"GPU memory after cache clear: {torch.cuda.memory_allocated() / 1e9:.2f} GB allocated")
        ...
        plot_concepts_feature_viz(
            ...
            crops_per_iteration=args.feature_viz_crops_per_iteration,
            total_steps=args.feature_viz_total_steps,
        )
