"""
CRAFT on DETR with PyTorch - Multiple Images

This script demonstrates CRAFT (Concept-based Recursive Activation Factorization)
on a DETR (Detection Transformer) object detection model with multiple giraffe images.

The images are loaded from a pre-processed .npz file (giraffes.npz) which contains
50 giraffe images already resized to 800x800 pixels - which perfectly matches DETR's
input size!
"""
...
# Output directory
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "torch_detr_craft")
...
images_size = (800, 800)  # DETR input size
transform = T.Compose(
    [
        T.Resize(images_size),
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
    ]
)
input_tensors = [transform(img).unsqueeze(0) for img in raw_images]
input_tensor = torch.cat(input_tensors, dim=0).to(device)  # Shape: (N, 3, 800, 800)
...
# ============================================================================
print("\n" + "=" * 80)
print("3. Displaying Sample Images with Predicted Bounding Boxes")
print("=" * 80)
...
# Build latent extractor (outputs boxes in absolute pixel coordinates)
batch_size = 1
latent_extractor = DetrExtractorBuilder.build(
    model=model, batch_size=batch_size, device=str(device), image_size=images_size
)
...
for idx in range(num_display_images):
    ...
    boxes = results[idx].filter(class_id=class_id, confidence=0.7)

    # Display image with boxes (boxes are in absolute pixel coordinates)
...
# # DEBUG: Exit early to check box alignment
# import sys
# sys.exit(0)
...
box_to_display = single_result[0].filter(class_id=class_id, confidence=0.5)
print(f"Number of boxes for '{class_name}': {box_to_display.shape[0]}")

# Display the raw image with boxes overlaid (boxes are in absolute pixel coordinates)
display_image = raw_images[display_image_idx]
...
importances_gi = craft.estimate_importance(
    input_tensor, operator, class_id, confidence=0.5, method="gradient_input"
)
