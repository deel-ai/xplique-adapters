"""
Attribution Methods on DETR with PyTorch

This script demonstrates multiple attribution methods on a DETR (Detection Transformer) object detection model.
...
confidence_threshold = 0.5  # Confidence threshold for filtering boxes
...
ATTRIBUTION_METHODS = {
    ...
    "squaregrad": False,  # Squared gradients
    "vargrad": False,  # Variance of gradients
    "smoothgrad": False,  # Smooth gradients (averaged over noisy samples)
    "occlusion": False,  # Perturbation-based attribution
    "hsic": False,  # HSIC attribution
    "sobol": False,  # Sobol attribution
    "rise": False,  # RISE attribution
}
...
# Output directory
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "torch_detr_attributions")
...
# DETR uses 800x800 input (perfect match!)
attribution_size = (800, 800)
print(f"Using {attribution_size} for DETR (matches npz file size)")
...
# ============================================================================
print("\n" + "=" * 80)
print("2. Loading DETR Model")
print("=" * 80)

print("Loading DETR ResNet-50 model from torch hub...")
model = torch.hub.load("facebookresearch/detr", "detr_resnet50", pretrained=True).to(
    device
)
model.eval()
print("DETR model loaded successfully")

# DETR uses COCO class names
COCO_CLASSES = [
...
print(f"Loaded {nb_classes} COCO class names")
class_id = CLASSES.index(class_name)

# DETR preprocessing - uses ImageNet normalization
transform = T.Compose(
    [
        T.Resize(attribution_size),
        T.ToTensor(),
...
# ============================================================================
# 3. Run Detection and Filter Boxes
# ============================================================================
print("\n" + "=" * 80)
print("3. Running Detection and Filtering Boxes")
print("=" * 80)

# Wrap model for detection
od_model = DetrBoxesModelWrapper(model, image_size=attribution_size)

# Get predictions for all images
all_filtered_boxes = []
for idx, processed_input in enumerate(processed_inputs):
    with torch.no_grad():
        predictions = od_model(processed_input)

    # predictions is a MultiBoxTensor, filter by class and confidence
...
# ============================================================================
# 4. Save Detection Visualizations
# ============================================================================
print("\n" + "=" * 80)
...
for img_idx, (raw_image, filtered_boxes) in enumerate(
    ...
    if os.path.exists(output_path):
        print(f"Image {img_idx}: Skipping (already exists)")
        continue
...
# Build DETR latent extractor for gradient-based attributions
# This bypasses DETR's in-place operations that conflict with gradient tracking
print("Building DETR latent extractor for attribution methods...")
latent_model = DetrExtractorBuilder.build(
    model, device=str(device), image_size=attribution_size
)
latent_model.output_as_tensor = True
print("DETR latent extractor built successfully!")

# Wrap with TorchWrapper for attribution
# Use is_channel_first=True since we'll provide channel-first PyTorch tensors
torch_wrapped_model = TorchWrapper(
    latent_model, device=device, is_channel_first=True, requires_grad=True
)

# Initialize explainers with latent model wrapper
explainers = {}
if any(ATTRIBUTION_METHODS.values()):
...
# Convert processed inputs to numpy format for latent extractor
# Input: PyTorch tensor, channel-first (N, C, H, W), normalized
# Output: numpy array, channel-last (N, H, W, C), normalized
# The latent extractor expects numpy arrays
numpy_inputs = []
for processed_input in processed_inputs:
    # Convert to numpy and transpose from (N, C, H, W) to (N, H, W, C)
    np_input = processed_input.cpu().detach().numpy().transpose(0, 2, 3, 1)
    numpy_inputs.append(np_input)

print(f"Prepared {len(numpy_inputs)} numpy inputs for attribution")
print(f"Input format: channel-last (N, H, W, C)")
print(f"Input shape: {numpy_inputs[0].shape}")
print(f"Input dtype: {numpy_inputs[0].dtype}")

# Process each image and each box
for img_idx, (raw_image, numpy_input, filtered_boxes) in enumerate(
    zip(raw_images, numpy_inputs, all_filtered_boxes)
):
    print(f"\n{'-' * 80}")
    ...
    # Convert to TensorFlow format (channel-last, as expected by latent extractor)
    tf_inputs = tf.convert_to_tensor(numpy_input, dtype=tf.float32)
    ...
    for enumerate_idx, actual_box_idx in enumerate(box_indices):
        ...
        for method_name, explainer in explainers.items():
            ...
            if os.path.exists(output_path):
                print(f"    - {config['name']}: Skipping (already exists)")
                continue
