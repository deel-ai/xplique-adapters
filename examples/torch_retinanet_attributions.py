"""
Attribution Methods on RetinaNet with PyTorch

This script demonstrates multiple attribution methods on a RetinaNet object detection model.
...
# This prevents TF from pre-allocating all available GPU memory,
# allowing PyTorch and TensorFlow to share GPU memory gracefully.
# Without this, TensorFlow may grab 8-9 GB upfront, causing OOM
# when PyTorch needs memory for gradient computations.
...
ATTRIBUTION_METHODS = {
    "saliency": False,  # Gradient-based attribution
    "gradientinput": False,  # Gradient × Input
...
# Output directory
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "torch_retinanet_attributions")
...
# Load the .npz file and get first N images
...
# (the reference script uses img.jpg which is 640x462, not 800x800)
attribution_size = (640, 480)
print(f"Resizing to {attribution_size} to reduce memory usage for attribution")
...
# ============================================================================
print("\n" + "=" * 80)
print("2. Loading RetinaNet Model")
print("=" * 80)
...
# ============================================================================
# 3. Preprocess Images and Get Predictions
# ============================================================================
print("\n" + "=" * 80)
print("3. Preprocessing Images and Getting Predictions")
print("=" * 80)

# RetinaNet preprocessing (following the original script pattern)
# Convert PIL to tensor, then apply preprocessing
...
for i, raw_image in enumerate(raw_images):
    image_tensor = pil_to_tensor(raw_image).unsqueeze(0)
    processed_input = preprocess(image_tensor).to(device)
    processed_inputs.append(processed_input)

print(f"Processed {len(processed_inputs)} images, shape: {processed_inputs[0].shape}")

# Create model wrapper (once for all images)
od_model = TorchvisionBoxesModelWrapper(model, nb_classes=nb_classes)
od_model.eval()

# Get predictions for each image
for i, processed_input in enumerate(processed_inputs):
    results = od_model(processed_input)
    filtered_boxes = results[0].filter(
        class_id=class_id, confidence=confidence_threshold
    )
    all_filtered_boxes.append(filtered_boxes)
    print(f"Image {i}: {filtered_boxes.shape[0]} '{class_name}' boxes detected")
...
# ============================================================================
# 5. Compute Attributions on Multiple Boxes
# ============================================================================
print("\n" + "=" * 80)
print(f"5. Computing Attributions on {num_boxes_per_image} Boxes per Image")
print("=" * 80)

# Print which methods are enabled
enabled_methods = [k for k, v in ATTRIBUTION_METHODS.items() if v]
print(f"Enabled methods: {', '.join(enabled_methods)}")

# Set model for attribution
od_model.output_as_tensor = True

# Create explainers for enabled methods only
explainers = {}
if any(ATTRIBUTION_METHODS.values()):
    ...
    if ATTRIBUTION_METHODS["vargrad"]:
        explainers["vargrad"] = VarGrad(
            ...
            operator=xplique.Tasks.OBJECT_DETECTION,
            batch_size=1,
            nb_samples=50,  # Number of noisy samples for variance
            noise=0.15,  # Noise level
        )
    ...
    if ATTRIBUTION_METHODS["smoothgrad"]:
        explainers["smoothgrad"] = SmoothGrad(
            ...
            operator=xplique.Tasks.OBJECT_DETECTION,
            batch_size=1,
            nb_samples=50,  # Number of noisy samples for averaging
            noise=0.15,  # Noise level
        )
    ...
    if ATTRIBUTION_METHODS["rise"]:
        explainers["rise"] = Rise(
            ...
            operator=xplique.Tasks.OBJECT_DETECTION,
            batch_size=1,
            nb_samples=4000,  # Number of random masks
        )
...
# Initialize timing tracking for each method
...
for img_idx, (raw_image, processed_input, filtered_boxes) in enumerate(
    ...
    # Check if there's at least one box
...
if not attribution_files:
    ...
    for method_name in methods_with_files:
        ...
        for col_idx, (img_idx, box_idx) in enumerate(image_box_pairs):
            ...
            if col_idx == 0:
                # Only show timing if this method was computed in this session
                if method_timings[method_name]:
                    mean_time = np.mean(method_timings[method_name])
                    print(f"  {config['name']} average time: {mean_time:.2f}s over {len(method_timings[method_name])} runs : {[f'{t:.2f}s' for t in method_timings[method_name]]}")
                    label_text = f"{config['name']}\n({mean_time:.2f}s)"
                else:
                    # Method not computed in this session, don't show timing
