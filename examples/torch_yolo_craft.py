"""
CRAFT on YOLO with PyTorch - Multiple Images

This script demonstrates CRAFT (Concept-based Recursive Activation Factorization)
on a YOLO object detection model with multiple giraffe images.

The images are loaded from a pre-processed .npz file (giraffes.npz) which contains
50 giraffe images already resized to 640x640 pixels.

Note: YOLO requires Semi-NMF (overcomplete factorizer) instead of standard NMF
because YOLO activations can contain negative values. Standard NMF requires
non-negative inputs, while Semi-NMF can handle both positive and negative values.
"""
...
# Output directory
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "torch_yolo_craft")
...
# YOLO preprocessing
images_size = (640, 640)
transform = T.Compose([T.Resize(images_size), T.ToTensor()])
input_tensors = [transform(img).unsqueeze(0) for img in raw_images]
input_tensor = torch.cat(input_tensors, dim=0).to(device)  # Shape: (N, 3, 640, 640)
...
latent_extractor = YoloExtractorBuilder.build(
    detection_model, extraction_layer=extraction_layer, batch_size=1
)
...
for idx in range(num_display_images):
    ...
    boxes = results[idx].filter(class_id=class_id, confidence=0.5)

    # Get the raw image and resize to match YOLO input size (boxes are in 640x640 coordinates)
    img_to_display = raw_images[idx].resize(images_size)
...
# YOLO requires Semi-NMF factorizer because activations can have negative values
num_concepts = 10
factorizer = OvercompleteFactorizer(
    optimizer_class=SemiNMF, nb_concepts=num_concepts, device=device
)
